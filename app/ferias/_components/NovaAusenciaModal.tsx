...
export function NovaAusenciaModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-lg shadow-xl flex flex-col overflow-hidden border border-line dark:border-slate-800">
        <div className="p-6 border-b border-line dark:border-slate-800 flex justify-between items-start bg-navy-50/50 dark:bg-slate-950/50">
          ...
          <button onClick={onClose} disabled={salvando} className="text-ink-400 hover:text-navy-700 dark:hover:text-white p-2 disabled:opacity-50">✕</button>
        </div>

        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1">Colaborador</label>
            <select
              value={respId}
              onChange={e => setRespId(e.target.value)}
              disabled={salvando || respLockado}
              className="w-full bg-transparent border border-line dark:border-slate-800 dark:text-white rounded-md px-3 py-3 text-sm outline-none focus:border-teal-500 disabled:opacity-60 disabled:cursor-not-allowed"
            >
            ...
            {respLockado && !isAdmin && (
              <p className="text-[11px] text-ink-400 mt-1">Você só pode cadastrar a própria ausência. Pra outras pessoas, pede pro admin.</p>
            )}
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1">Início</label>
              <input
                type="date"
                value={dataInicio}
            ...
            <div>
              <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1">Fim (inclusivo)</label>
              <input
                type="date"
                value={dataFim}
          ...
          <div>
            <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1">Motivo</label>
            <select
              value={motivo}
          ...
          <div>
            <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1 flex items-center gap-1.5">
          ...
          <div>
            <label className="text-xs text-ink-500 dark:text-slate-400 font-medium block mb-1">Observação (opcional)</label>
            <textarea
              value={obs}
              onChange={e => setObs(e.target.value)}
              rows={3}
              disabled={salvando}
              className="w-full bg-transparent border border-line dark:border-slate-800 dark:text-white rounded-md px-3 py-3 text-sm outline-none focus:border-teal-500 disabled:opacity-50 resize-none"
        ...
        <div className="p-5 border-t border-line dark:border-slate-800 flex justify-end gap-2 bg-navy-50 dark:bg-slate-950">
          <button
            onClick={onClose}
            disabled={salvando}
            className="px-5 py-2.5 rounded-md text-sm font-semibold text-ink-500 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            className="bg-navy-700 hover:bg-[#042436] text-white px-5 py-2.5 rounded-md text-sm font-semibold transition-colors shadow-sm disabled:opacity-50"
          >
