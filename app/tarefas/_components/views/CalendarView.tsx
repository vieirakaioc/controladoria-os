...
export function CalendarView({ data, abrirDrawer }: Props) {
  return (
    <main className="bg-white dark:bg-slate-900 rounded-lg shadow-card border border-line dark:border-slate-800 overflow-hidden transition-colors">
      <div className="grid grid-cols-7 border-b border-line dark:border-slate-800 bg-navy-700 dark:bg-slate-950">
        {HEADERS.map(d => (
          <div key={d} className="p-3 text-center text-xs font-bold text-white uppercase tracking-wider">{d}</div>
        ))}
      </div>
      <div className="grid grid-cols-7 auto-rows-fr bg-navy-100 dark:bg-slate-800 gap-[1px]">
        {data.diasVaziosInicio.map(id => (
          <div key={id} className="min-h-[140px] bg-navy-50/50 dark:bg-slate-900/50" />
        ))}

        {data.diasDoMes.map(diaInfo => (
          <div
            key={diaInfo.dia}
            className={`min-h-[140px] p-2 bg-white dark:bg-slate-900 flex flex-col transition-colors hover:bg-navy-50 dark:hover:bg-slate-800/80 ${diaInfo.isHoje ? 'ring-2 ring-inset ring-[#0f88a8] dark:ring-[#38bdf8] bg-teal-600/5 dark:bg-[#0f88a8]/10' : ''}`}
          >
            <div className="flex justify-between items-start mb-2">
              <span className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${diaInfo.isHoje ? 'bg-teal-600 dark:bg-[#38bdf8] text-white shadow-sm' : 'text-ink-700 dark:text-slate-400'}`}>
                {diaInfo.dia}
              </span>
              {diaInfo.tarefas.length > 0 && (
                <span className="text-[10px] font-bold text-ink-400 dark:text-slate-300 bg-navy-100 dark:bg-slate-800 px-1.5 rounded">{diaInfo.tarefas.length}</span>
              )}
            </div>
