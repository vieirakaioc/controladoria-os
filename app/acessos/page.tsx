...
export default function AcessosPage() {
  ...
  if (loading) return <div className="min-h-screen bg-navy-50 dark:bg-slate-950 flex items-center justify-center text-teal-600 dark:text-[#38bdf8] font-medium animate-pulse transition-colors">A verificar credenciais de segurança...</div>
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-navy-50 dark:bg-slate-950 p-8 flex flex-col items-center justify-center text-center transition-colors">
        ...
        <p className="text-ink-500 dark:text-slate-400 mt-2 max-w-md">Esta área é um cofre de segurança. Apenas administradores do portal podem gerir os acessos da equipa.</p>
        <button onClick={() => router.push('/tarefas')} className="mt-6 bg-teal-600 hover:bg-[#0c708b] dark:hover:bg-[#0284c7] transition-colors text-white px-6 py-2.5 rounded-md font-medium shadow-sm">
          Voltar ao Kanban
        </button>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-navy-50 dark:bg-slate-950 p-8 font-sans transition-colors duration-300">
      ...
      <header className="mb-8 bg-white dark:bg-slate-900 p-6 rounded-lg shadow-card border border-line dark:border-slate-800 flex items-center gap-4 transition-colors">
      ...
      <main className="bg-white dark:bg-slate-900 rounded-lg shadow-card border border-line dark:border-slate-800 overflow-hidden transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-navy-50 dark:bg-slate-950 transition-colors">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
              {profiles.map((p) => (
                <tr key={p.id} className={`transition-colors text-sm ${p.id === currentUserId ? 'bg-teal-600/5 dark:bg-[#38bdf8]/10' : 'hover:bg-navy-50 dark:hover:bg-slate-800/50'}`}>
                  ...
                  {/* ID */}
                  <td className="p-4 text-ink-400 dark:text-slate-500 font-mono text-xs">
                    {p.id.split('-')[0]}...
                  </td>
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-md text-[10px] tracking-wide uppercase font-bold ${
                      p.role === 'admin' ? 'bg-navy-700 dark:bg-[#38bdf8]/20 text-white dark:text-[#38bdf8]' : 'bg-navy-100 dark:bg-slate-800 text-ink-700 dark:text-slate-300'
                    }`}>
                      {p.role}
                    </span>
                  </td>
          ...
          {profiles.length === 0 && (
            <div className="p-12 text-center text-ink-500 dark:text-slate-400">Nenhum perfil encontrado.</div>
          )}
        </div>
      </main>
