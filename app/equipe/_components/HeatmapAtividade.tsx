...
function corCell(valor: number, max: number) {
  if (max === 0 || valor === 0) return 'bg-navy-100 dark:bg-slate-800/40'
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-card border border-line dark:border-slate-800 p-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          {pico.valor > 0 && (
            <div className="text-xs text-ink-700 dark:text-slate-400 bg-navy-50 dark:bg-slate-800/50 rounded-md px-3 py-2 border border-line dark:border-slate-700">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-card border border-line dark:border-slate-800 p-6">
        <h4 className="text-sm font-bold text-navy-700 dark:text-white uppercase tracking-wider mb-4">
            ...
            <div className="flex items-center pl-12 mb-1">
              {Array.from({ length: 24 }).map((_, h) => (
                <div key={h} className="w-6 text-center">
                  <span className={`text-[9px] font-mono ${h % 3 === 0 ? 'text-ink-500 dark:text-slate-400' : 'text-ink-400 dark:text-slate-600'}`}>
            ...
            {DIAS.map((dia, d) => (
              <div key={dia} className="flex items-center mb-0.5">
                <div className="w-12 pr-3 text-right">
                  <span className="text-xs font-bold text-ink-700 dark:text-slate-300">{dia}</span>
                </div>
        ...
        {/* Legenda de cor */}
        <div className="mt-5 flex items-center gap-2 text-xs text-ink-500 dark:text-slate-400">
          <span>Menos</span>
          <div className="w-4 h-4 rounded bg-navy-100 dark:bg-slate-800/40" />
              ...
              return (
                <div key={c.email} className="flex items-center gap-3 py-1">
                  <div className="w-48 truncate">
                    <span className="text-sm font-medium text-ink-700 dark:text-slate-200">{c.email}</span>
                  </div>
                  ...
                  <div className="w-16 text-right">
                    <span className="text-xs font-bold tabular-nums text-ink-700 dark:text-slate-200">{c.total}</span>
                  </div>
                </div>
              )
            })}
